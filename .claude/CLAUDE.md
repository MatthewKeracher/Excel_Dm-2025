...
| `src/editor.js` | CodeMirror-based modal editor; snippet panel with "Snippets" and "Rules" tabs. Toolbar includes a yellow **H** highlighter button (wraps selection in `<mark>`) and a **# Field** button that inserts a `<span data-field="NAME">0</span>` sentinel for the inline-fields feature. The Rules pane's `makeEntryRow` honours an optional `extraButtons(entry, cache)` callback on each section config — used by BFRPG monsters to expose **W**/**L** Wild/Lair encounter-table buttons. |
| `src/dialog.js` | In-app modal dialogs — `alertDialog()`, `confirmDialog()`, `promptDialog()`. Promise-based and stack-aware so nested dialogs don't collide. Replaces every `window.confirm/alert/prompt` (the editor exit prompt, delete-confirm dialogs, add-tab prompt, etc.). Styled via `.app-dialog-*` classes in `style.css`. |
...
| `src/rulesets/{Name}/` | Per-ruleset plugin directory. For BFRPG: `index.js` exports `sections` (drives Rules pane; monsters section provides `extraButtons` returning **W**/**L** encounter-table buttons when `appearing` parses out wild/lair counts) and `generators` (drives Treasure panel); `formatters.js` formats monsters/spells/items (monster block + one-liner emit `<span data-field="hp">` so DMs can edit rolled HP in play; the one-liner also includes `#At` and `XP`; **`formatMonsterEncounterTable`** rolls a Wild or Lair count and emits a BFRPG-module-style block — header line with shared stats, then a 2-column run of `<HP> <span data-field="hp-tally">` clusters with persistent clickable damage boxes — plus a single group-treasure line at the end. Three "Insert as Table" formatters (`formatMonsterTable`, `formatSpellTable`, `formatItemTable`) emit a leading `Qty` column wired to inline fields. **Pack** items (category `packs`) carry a `contents:[{name, qty}]` array; their block/one-liner expand into a Qty/Name/Cost/Weight markdown table with cost/weight looked up from the items catalog); `npc.js` generates NPC stat blocks (same editable-HP treatment); `treasure.js` rolls gems/jewelry/magic items/potions/scrolls — `rollIndividualTreasure(code, spells)` for one monster, `rollGroupTreasure(code, count, spells)` for an encounter (cash totals are summed; gems/jewelry/magic items list individually) |
| `src/notecard.js` | Notecard factory — `makeNoteCard()`, `makePopOut()`, `loadPopUp()`; all button creation and card assembly. Wires `inlineFields.js` into rendered bodies. The pin (🔒/🔓) button is available on every card type and toggles the per-browser pinned state. Picking a colour on a **quest** notecard recolours the entire questline (root + all descendants); other entry types only recolour the single entry. |
| `src/inlineFields.js` | `wireInlineFields(bodyEl, entry, opts)` — walks `<span data-field="NAME">value</span>` sentinels in a rendered markdown body, swaps each for an editable `<input>`, and rewrites the matching span inside `entry.body` on change. HTML-escapes user input. Multiple fields per card are addressed by occurrence index. Used by both notecards (`notecard.js`) and home notices (`home.js`); skipped for anon users and disabled for viewer role. **Special field type `data-field="hp-tally"` with a `data-max="N"` attribute** renders as a row of N clickable damage-tracker boxes; the source span's text is the count of ticked boxes (0..N) and ticks persist via the same PATCH path. |
...
| `src/left.js` | Renders note cards in the left sidebar using `notecard.js`; card click, delete, navigation. Quests are sorted by **progress depth** (deepest visible objective first via `countParentsUp()`) so the questlines that have advanced furthest float to the top. |
...
`main.go` wraps `http.FileServer` with `noCacheStatic`, which sets `Cache-Control: no-cache` on every `.html`, `.js`, `.css`, `.json`, and `/` response. That forces both the browser and Cloudflare's edge to revalidate against origin every request (Cloudflare reports `cf-cache-status: DYNAMIC`), so frontend deploys take effect on the next request — no waiting for TTL expiry. The `/invite/{token}` SPA route still sets `Cache-Control: no-store` explicitly (stricter — never store at all). Image, font, and other binary assets are still served plain so Cloudflare can cache them.
...
- `data/BFRPG/items.json` — 435 items (structured: `name`, `category`, `cost`, `weight`, `damage`, `ac`, `size`, `description`, `oneLiner`). Items in the `packs` category additionally carry a `contents: [{ name, qty }]` array — each `name` matches an existing item entry so the pack formatter can render a Qty/Name/Cost/Weight table with cost/weight populated from the catalog.
...
- **BFRPG integration:** monster blocks, monster one-liners, NPC blocks, and NPC one-liners all emit `<span data-field="hp">...</span>` so the rolled HP is per-instance editable for live combat tracking. The Wild/Lair encounter formatter (`formatMonsterEncounterTable`) emits `<span data-field="hp-tally" data-max="N">0</span>` per individual — clickable damage-tracker boxes that persist via the same PATCH path. Insert-as-Table buttons emit a `<span data-field="qty">1</span>` per row for live quantity tracking.
